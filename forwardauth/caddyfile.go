...
// as the custom_reverse_proxy directive (in fact, the custom_reverse_proxy's directive
...
//	}
//
// is equivalent to a custom_reverse_proxy directive like this:
//
//	custom_reverse_proxy auth-gateway:9091 {
//	    method GET
...
//	        request_header {
//	            Remote-User {http.custom_reverse_proxy.header.Remote-User}
//	            Remote-Email {http.custom_reverse_proxy.header.Remote-Email}
//	        }
//	    }
//	}
func parseCaddyfile(h httpcaddyfile.Helper) ([]httpcaddyfile.ConfigValue, error) {
	...
	// we can gain the rest of the custom_reverse_proxy syntax
	...
	// custom_reverse_proxy unmarshaler to read from it as well
	...
	// NOTE: we delete the tokens as we go so that the custom_reverse_proxy
	...
	for dispenser.Next() {
		for dispenser.NextBlock(0) {
			...
			// the custom_reverse_proxy passthrough tokens
	...
	// reset the dispenser after we're done so that the custom_reverse_proxy
	...
	// logic in custom_reverse_proxy to not skip this entry as empty.
	for from, to := range headersToCopy {
		handler.Request.Set[to] = []string{
			"{http.custom_reverse_proxy.header." + from + "}",
		}
	}
	...
	// using the custom_reverse_proxy directive syntax
