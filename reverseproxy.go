...
// ------------|-------------
// `{http.custom_reverse_proxy.upstream.address}` | The full address to the upstream as given in the config
// `{http.custom_reverse_proxy.upstream.hostport}` | The host:port of the upstream
// `{http.custom_reverse_proxy.upstream.host}` | The host of the upstream
// `{http.custom_reverse_proxy.upstream.port}` | The port of the upstream
// `{http.custom_reverse_proxy.upstream.requests}` | The approximate current number of requests to the upstream
// `{http.custom_reverse_proxy.upstream.max_requests}` | The maximum approximate number of requests allowed to the upstream
// `{http.custom_reverse_proxy.upstream.fails}` | The number of recent failed requests to the upstream
// `{http.custom_reverse_proxy.upstream.latency}` | How long it took the proxy upstream to write the response header.
// `{http.custom_reverse_proxy.upstream.latency_ms}` | Same as 'latency', but in milliseconds.
// `{http.custom_reverse_proxy.upstream.duration}` | Time spent proxying to the upstream, including writing response body to client.
// `{http.custom_reverse_proxy.upstream.duration_ms}` | Same as 'upstream.duration', but in milliseconds.
// `{http.custom_reverse_proxy.duration}` | Total time spent proxying, including selecting an upstream, retries, and writing response.
// `{http.custom_reverse_proxy.duration_ms}` | Same as 'duration', but in milliseconds.
type Handler struct {
	// Configures the method of transport for the proxy. A transport
	// is what performs the actual "round trip" to the backend.
	// The default transport is plaintext HTTP.
	TransportRaw json.RawMessage `json:"transport,omitempty" caddy:"namespace=http.custom_reverse_proxy.transport inline_key=protocol"`
	...
	CBRaw json.RawMessage `json:"circuit_breaker,omitempty" caddy:"namespace=http.custom_reverse_proxy.circuit_breakers inline_key=type"`
	...
	DynamicUpstreamsRaw json.RawMessage `json:"dynamic_upstreams,omitempty" caddy:"namespace=http.custom_reverse_proxy.upstreams inline_key=source"`
	...
	// - `{http.custom_reverse_proxy.status_code}` The status code from the response
	// - `{http.custom_reverse_proxy.status_text}` The status text from the response
	// - `{http.custom_reverse_proxy.header.*}` The headers from the response
...
func (Handler) CaddyModule() caddy.ModuleInfo {
	return caddy.ModuleInfo{
		ID:  "http.handlers.custom_reverse_proxy",
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	defer func() {
		...
		repl.Set("http.custom_reverse_proxy.duration", time.Since(start))
		repl.Set("http.custom_reverse_proxy.duration_ms", time.Since(start).Seconds()*1e3) // multiply seconds to preserve decimal (see #4666)
	}()
...
func (h *Handler) proxyLoopIteration(r *http.Request, origReq *http.Request, w http.ResponseWriter, proxyErr error, start time.Time,
	...
	repl.Set("http.custom_reverse_proxy.upstream.address", dialInfo.String())
	repl.Set("http.custom_reverse_proxy.upstream.hostport", dialInfo.Address)
	repl.Set("http.custom_reverse_proxy.upstream.host", dialInfo.Host)
	repl.Set("http.custom_reverse_proxy.upstream.port", dialInfo.Port)
	repl.Set("http.custom_reverse_proxy.upstream.requests", upstream.Host.NumRequests())
	repl.Set("http.custom_reverse_proxy.upstream.max_requests", upstream.MaxRequests)
	repl.Set("http.custom_reverse_proxy.upstream.fails", upstream.Host.Fails())
	...
	proxyErr = h.customReverseProxy(w, r, origReq, repl, dialInfo, next)
...
func (h *Handler) customReverseProxy(rw http.ResponseWriter, req *http.Request, origReq *http.Request, repl *caddy.Replacer, di DialInfo, next caddyhttp.Handler) error {
	...
	repl.Set("http.custom_reverse_proxy.upstream.latency", duration)
	repl.Set("http.custom_reverse_proxy.upstream.latency_ms", duration.Seconds()*1e3) // multiply seconds to preserve decimal (see #4666)
	...
	for i, rh := range h.HandleResponse {
		...
		for field, value := range res.Header {
			repl.Set("http.custom_reverse_proxy.header."+field, strings.Join(value, ","))
		}
		repl.Set("http.custom_reverse_proxy.status_code", res.StatusCode)
		repl.Set("http.custom_reverse_proxy.status_text", res.Status)
...
func (h Handler) finalizeResponse(
	...
	repl.Set("http.custom_reverse_proxy.upstream.duration", time.Since(start))
	repl.Set("http.custom_reverse_proxy.upstream.duration_ms", time.Since(start).Seconds()*1e3)
...
type LoadBalancing struct {
	...
	SelectionPolicyRaw json.RawMessage `json:"selection_policy,omitempty" caddy:"namespace=http.custom_reverse_proxy.selection_policies inline_key=policy"`
...
const proxyHandleResponseContextCtxKey caddy.CtxKey = "custom_reverse_proxy_handle_response_context"
