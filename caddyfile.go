	...
	for d.Next() {
		...
		for d.NextBlock(0) {
			...
			case "dynamic":
				...
				dynModule := d.Val()
				modID := "http.custom_reverse_proxy.upstreams." + dynModule
			...
			case "lb_policy":
				...
				name := d.Val()
				modID := "http.custom_reverse_proxy.selection_policies." + name
				unm, err := caddyfile.UnmarshalModule(d, modID)
				if err != nil {
					return err
				}
				sel, ok := unm.(Selector)
				if !ok {
					return d.Errf("module %s (%T) is not a customreverseproxy.Selector", modID, unm)
				}
			...
			case "transport":
				...
				modID := "http.custom_reverse_proxy.transport." + transportModuleName
				unm, err := caddyfile.UnmarshalModule(d, modID)
				if err != nil {
					return err
				}
